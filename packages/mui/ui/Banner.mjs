...
export const NeoBanner = React.forwardRef(function NeoBanner({
  ...
  const warn = (m) => {
    if (process.env.NODE_ENV !== 'production') console.warn(`[Neo] Banner: ${m}`);
  };
  ...
  if (['large', 'medium'].includes(size) && !media && !src) warn('without an image —`media` or `src`— the banner still reserves its image half and it stays blank: the master draws it in all its variants.');
