...
These are the packages: `@neo-design/foundations` resolves to `foundations/` and `@neo-design/mui` to `mui/`, and neither to any other folder (the repo is organized by the role each file plays). What ties them together is not
