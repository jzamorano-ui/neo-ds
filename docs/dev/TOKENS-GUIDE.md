...
**Width and height** — the system tokenizes a size only when two pieces have to share it to line up: `icon/size/*`, the three button heights (`--neo-button-height-sm|md|lg`), the shared control height (`--neo-input-height`, 44 — text field, select and combobox), the switch (`--neo-switch-width|height`), the menu (`--neo-menu-min-width`, `--neo-menu-item-min-height`) and `--neo-radio-dot`. Use those instead of writing the number: align your own control to a field with `var(--neo-input-height)`. What is **not** tokenized is arbitrary size: the width of a card, a panel or a column is **layout**, not drift — it is solved with the grid and the container.
...
```js
// JS — from the package
import { color, colorVars, scale } from '@neo-design/mui';
