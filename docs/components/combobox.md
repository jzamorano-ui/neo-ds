...
| gap (separador) | border (inside the `combobox-field`) | `--focus--gap--default` |
...
- **The theme is not enough on its own: there is a composition layer on top.** Dressing MUI's `Autocomplete` solves the color and the box, but three things in this spec live **above** the theme and are assembled by whoever consumes it: the **static `label`** (it goes inside `renderInput` — MUI's floating label does not work here), the **`focus` / `active` / `writing` distinction**, and the **ring by modality** of the previous rule. How it is encapsulated —your own component, a hook, nothing— is dev's decision; that the three **are there** is not optional. It is the same composition the components map marks as "static label".
